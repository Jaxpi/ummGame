const ALL_RULES = [
    "Must clap hands before speaking", "Cannot use words starting with 'S'", "Must speak in a high pitched voice",
    "Cannot look directly at other players", "Must end every sentence with 'mate'", "Must keep both elbows on the table",
    "Cannot use the word 'The'", "Must scratch your head when thinking", "Must answer questions with a whisper",
    "Cannot point fingers at anyone", "Must puff out cheeks while silent", "Cannot say the word 'And'",
    "Must nod constantly while talking", "Must keep one eye closed", "Cannot use contractions (don't, can't)",
    "Must speak like a pirate", "Must clear throat before every answer", "Cannot cross your legs",
    "Must say 'Beep' after mentioning a name", "Cannot say the word 'Uh' or 'Umm'", "Must touch your nose when answering",
    "Cannot use words with more than 3 syllables", "Must speak entirely in monotone", "Must say 'Wow' before every clue",
    "Cannot use your thumbs", "Must keep your tongue slightly out", "Cannot use the word 'Like'",
    "Must wave hello when a turn starts", "Cannot say 'Yes' or 'No'", "Must shrug your shoulders every 5 seconds",
    "Must talk without showing your teeth", "Cannot say the word 'You'"
];

// 60+ Authentic Playable Adjectives
const ADJECTIVES = [
    "angry", "sticky", "smelly", "wobbly", "shiny", "greasy", "fluffy", "frozen", "spicy", "gigantic",
    "tiny", "golden", "slippery", "noisy", "clumsy", "magic", "haunted", "stinky", "fancy", "fuzzy",
    "sweaty", "crumpy", "salty", "glowing", "rusty", "shaky", "muddy", "elastic", "sleepy", "screaming",
    "creepy", "heavy", "skinny", "plastic", "flying", "liquid", "fancy", "bubbly", "grumpy", "brave",
    "crazy", "lonely", "wooden", "chubby", "poisonous", "secret", "smoky", "jolly", "wild", "blind",
    "juicy", "chilly", "flaming", "melted", "filthy", "polite", "drunken", "nervous", "hungry", "ghostly",
    "rotten", "speedy"
];

// 60+ Authentic Playable Nouns
const NOUNS = [
    "hamster", "banana", "grandma", "toaster", "monkey", "penguin", "pirate", "octopus", "cactus", "diaper",
    "wizard", "chicken", "hotdog", "astronaut", "monster", "toilet", "mermaid", "hamburger", "broccoli", "donkey",
    "eyeball", "zombie", "pancake", "squirrel", "potato", "teacher", "dinosaur", "baby", "vampire", "balloon",
    "pillow", "sandwich", "tractor", "spider", "pickle", "waffle", "alien", "gorilla", "kitten", "robot",
    "chicken", "suitcase", "muffin", "sneaker", "garbage", "blanket", "pumpkin", "cowboy", "hippie", "doctor",
    "onion", "spider", "eyebrow", "burrito", "pigeon", "sausage", "shadow", "trophy", "feather", "guitar",
    "ladder", "trumpet"
];
...
function resetToHome(event) {
    ...
    pauseTimer();
    teamARules = []; teamBRules = []; teamAProgress = 0; teamBProgress = 0;
...
function initWordDecks() {
    ...
    if (localStorage.getItem('umm_pairing_history')) {
        combinationHistory = JSON.parse(localStorage.getItem('umm_pairing_history'));
    } else {
...
function drawNextPairing() {
    ...
    for (let a = 0; a < unusedAdjectives.length; a++) {
        ...
        for (let n = 0; n < unusedNouns.length; n++) {
            ...
            if (!combinationHistory[currentAdj] || !combinationHistory[currentAdj].includes(currentNoun)) {
                chosenAdjIndex = a;
                chosenNounIndex = n;
    ...
    if (!combinationHistory[targetAdjective]) combinationHistory[targetAdjective] = [];
    combinationHistory[targetAdjective].push(targetNoun);
    localStorage.setItem('umm_pairing_history', JSON.stringify(combinationHistory));
...
function startGameBoard() {
    teamAProgress = 0; teamBProgress = 0;
    resetTimer();
    buildTrackSquares('A');
    buildTrackSquares('B');
    const pCards = document.querySelectorAll('.penalty-card');
    pCards.forEach(c => { c.classList.remove('revealed', 'flashing'); c.innerHTML = '<span>Rule</span>'; });
